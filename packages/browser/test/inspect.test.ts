...
describe("browser inspection", () => {
  ...
  it.skipIf(!browserAvailable)("captures the golden movie fixture at all required viewports", async () => {
    ...
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "arqen-movie-evidence-"));
    const report = await inspectLocalFixture({ target: pathToFileURL(path.join(root, "index.html")).href, allowedRoot: root, outputDir });
    expect(report.evidence).toHaveLength(3);
    expect(report.evidence.map((item) => item.viewport.width)).toEqual([1440, 1024, 390]);
    expect(report.evidence.every((item) => item.accessibility.ariaSnapshot)).toBe(true);
    expect(report.evidence.every((item) => item.failedRequests.length === 0)).toBe(true);
    expect(report.evidence.find((item) => item.viewport.name === "mobile")?.overflow.horizontal).toBe(true);
    expect(report.evidence.every((item) => item.screenshot.endsWith(".png"))).toBe(true);
  });
