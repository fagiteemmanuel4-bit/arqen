...
export function applyPlan(root: string, plan: TransformationPlan): { applied: string[]; skipped: string[] } {
  ...
  for (const edit of plan.edits) {
    const target = path.resolve(rootPath, edit.path);
    if (target !== rootPath && !target.startsWith(`${rootPath}${path.sep}`)) throw new Error(`Unsafe transformation path: ${edit.path}`);
    ...
    try {
      fs.writeFileSync(target, edit.after, "utf8");
...
export function rollbackPlan(root: string, plan: TransformationPlan): string[] {
  ...
  for (const edit of plan.edits) {
    const target = path.resolve(rootPath, edit.path);
    if (!target.startsWith(`${rootPath}${path.sep}`)) throw new Error(`Unsafe rollback path: ${edit.path}`);
    const backup = `${target}.arqen-backup`;
...
export function buildSafeAuditPlan(project: Project, audit: AuditResult): TransformationPlan {
  ...
  const tokenFinding = audit.findings.find((finding) => finding.rule === "system.no-tokens");
  if (tokenFinding) {
    const cssFile = project.files.find((file) => file.extension === ".css" && file.path.startsWith("src/"));
    if (cssFile) {
      const absolute = path.join(project.root, cssFile.path);
